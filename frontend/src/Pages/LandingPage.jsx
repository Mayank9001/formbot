    ...
    <div className={styles.container}>
      ...
      <footer className={styles.landingFooter}>
        <div className={styles.logoFooter}>
          ...
          <p>
            Made with ❤️ by <br />Mayank Vishwakarma<br />mayankvishw09@gmail.com<br/ >
            <a href="https://github.com/Mayank9001" target="_blank" className={styles.git}>Github</a>
          </p>
        </div>
        <div className={styles.footProduct}>
          <p>Products</p>
          {["Status", "Documentation", "Roadmap", "Pricing"].map((prod, index) => (
            <li key={index}>{prod}<PiArrowSquareOut /></li>
          ))}
        </div>
        <div className={styles.footProduct}>
          <p>Community</p>
          {["Discord","GitHub repository","Twitter","LinkedIn","OSS Friends",
          ].map((com, index)=>( <li key={index}>{com}<PiArrowSquareOut /></li>))}
        </div>
        <div className={styles.footProduct}>
          <p>Company</p>
          {["About", "Contact", "Terms & Services","Privacy Policy"].map((cmp, index)=>(<li key={index}>{cmp}<PiArrowSquareOut /></li>))}
        </div>
      </footer>
    </div>
  );
